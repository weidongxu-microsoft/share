// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.compute;

import com.azure.core.util.Context;

/** Samples for CloudServiceOperatingSystems GetOSFamily. */
public final class CloudServiceOperatingSystemsGetOSFamilySamples {
    /**
     * Sample code: Get Cloud Service OS Family.
     *
     * @param computeManager Entry point to ComputeManager. Compute Client.
     */
    public static void getCloudServiceOSFamily(com.azure.resourcemanager.compute.ComputeManager computeManager) {
        computeManager.cloudServiceOperatingSystems().getOSFamilyWithResponse("westus2", "3", Context.NONE);
    }
}
